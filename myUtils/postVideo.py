...
def post_image_DouYin(title, files, tags, account_file, category=TencentZoneTypes.LIFESTYLE.value, enableTimer=False, videos_per_day=1, daily_times=None, start_days=0,
                      productLink='', productTitle='', music_name=''):
    # 导入自定义的DouYinImage类
    ...
    if isinstance(files, list):
        # 如果是多张图片，需要将它们作为一个列表传递给单个上传任务
        files_paths = [str(Path(BASE_DIR / "imageFile" / file)) for file in files]
    else:
        # 单张图片
        files_paths = [str(Path(BASE_DIR / "imageFile" / files))]
    
    if enableTimer:
        publish_datetimes = generate_schedule_time_next_day(1, videos_per_day, daily_times, start_days)  # 图片作为一个整体发布
    else:
        publish_datetimes = [0]
    
    for cookie in account_file:
        print(f"图片文件路径: {files_paths}")
        print(f"标题：{title}")
        print(f"Hashtag：{tags}")
        # 传递图片文件列表给DouYinImage
        app = DouYinImage(title, files_paths if len(files_paths) > 1 else files_paths[0], tags, publish_datetimes[0], cookie, productLink, productTitle, music_name)
        asyncio.run(app.main(), debug=False)
